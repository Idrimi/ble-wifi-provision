#!/usr/bin/env bash
#
# setup-ble-wifi-bookworm.sh — one-shot installer for Debian Bookworm Lite
#
set -euo pipefail
...
apt-get update
apt-get install -y \
  python3-trio python3-dbus-next \
  bluez bluetooth rfkill \
  network-manager

# If python3-dbus-next isn't available, fall back to pip:
if ! dpkg -s python3-dbus-next &>/dev/null; then
  echo "python3-dbus-next not in APT, installing via pip…"
  apt-get install -y python3-pip
  pip3 install dbus-next trio
fi
...
# 3) Write Python GATT server
echo "==> Deploying ble-creds-server.py…"
cat > /usr/local/bin/ble-creds-server.py <<'EOF'
#!/usr/bin/env python3
import sys, trio
